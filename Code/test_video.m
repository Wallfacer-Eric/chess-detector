...
while input('input a number: ') 
      ...
      if repeat_time==0
          ...
          original_image=imread('http://10.12.120.157:8080/image');
          for i=1:3
               original_image(:,:,i)=medfilt2(original_image(:,:,i),[7,7]);
          end
          [loc_x,loc_y]=get_location(original_image);
      elseif repeat_time==1
          %first_image= getsnapshot(vid);
          first_image=imread('http://10.12.120.157:8080/image');
          for i=1:3
                first_image(:,:,i)=medfilt2(first_image(:,:,i),[7,7]);
          end
          ...
          DrawAll(Board,Chess,Player,Basic,[]);
            ...
            test2_image=imread('http://10.12.120.157:8080/image');
            for i=1:3
